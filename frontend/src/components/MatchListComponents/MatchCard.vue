  ...
  <div class="card shadow-sm">
    <div class="card-body" @click="fixturesStore.toggleMatchDetails(match.id)">
      <div class="d-flex justify-content-between align-items-center">
        <span class="small fw-medium">Week {{ match.week || 'No Info' }}</span>
      ...
      <div v-if="match.status === 'completed'" class="mt-2 text-center">
        <span class="fw-bold text-success">🏆 Winner: {{ fixturesStore.getWinner(match) }}</span>
      </div>
      ...
      <div v-if="match.status === 'ongoing'" class="bg-light p-2 rounded small">
        <div class="d-flex justify-content-between align-items-center">
          <span class="text-muted">{{ match.current_minute }}/48 min</span>
          <span v-if="match.currentPossession" class="badge bg-success">
            Ball: {{ match.currentPossession }}
          </span>
        </div>
        <div v-if="match.lastAttack" class="mt-1 border-start border-warning ps-2">
          <p class="small mb-0">
            <span class="fw-semibold">{{ match.lastAttack.team_name }}</span>: {{ match.lastAttack.shooter }} {{ match.lastAttack.shot_type || (match.lastAttack.points + " points") }}
            <span :class="match.lastAttack.result === 'success' ? 'text-success' : 'text-danger'">
              {{ match.lastAttack.result === 'success' ? 'successful!' : 'missed' }}
            </span>
          </p>
        </div>
      </div>
      ...
      <div class="d-flex justify-content-between small mt-2">
        <span>
          ...
          {{ fixturesStore.expandedMatches[match.id] ? 'Close' : 'Details' }}
        </span>
      </div>
    </div>
  </div>
